...
class Node:
    ...
    def ca_prediction(self, instance):
        """
        :param instance: training instance
        :return: result dict
        """
        ...
        if not self.leaf:
            ...
            if self.data_type == 1:
                ...
                if instance[self.attr] > self.split_criterion:
                    return self.children[0].ca_prediction(instance)
                else:
                    return self.children[1].ca_prediction(instance)

            if self.data_type == 2:
                if 0 < len(np.where(instance[self.attr] == self.split_criterion)[0]):
                    clss = np.int(np.where(instance[self.attr] == self.split_criterion)[0])
                    return self.children[clss].ca_prediction(instance)
                else:
                    return self.result
        else:
            return self.result
    ...
    def evaluate(self, x, y, data_types):
        ...
        for i in range(10):
            ...
            predictions = [list(self.ca_prediction(row).keys())[0] for row in xtest]
